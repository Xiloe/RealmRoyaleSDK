...
public:
	...
	int STATIC_GetRandomOptionSumFrequency(TArray<float>* FreqList);
	int STATIC_GetBuildChangelistNumber();
	int STATIC_GetEngineVersion();
	void STATIC_GetSystemTime(int* Year, int* Month, int* DayOfWeek, int* Day, int* Hour, int* Min, int* Sec, int* MSec);
	struct FString STATIC_TimeStamp();
	struct FVector STATIC_TransformVectorByRotation(const struct FRotator& SourceRotation, const struct FVector& SourceVector, bool bInverse);
	struct FName STATIC_GetPackageName();
	bool STATIC_IsPendingKill();
	float STATIC_ByteToFloat(unsigned char inputByte, bool bSigned);
	unsigned char STATIC_FloatToByte(float inputFloat, bool bSigned);
	...
	float STATIC_PointDistToSegment(const struct FVector& Point, const struct FVector& StartPoint, const struct FVector& EndPoint, struct FVector* OutClosestPoint);
	float STATIC_PointDistToLine(const struct FVector& Point, const struct FVector& Line, const struct FVector& Origin, struct FVector* OutClosestPoint);
	...
	void STATIC_SaveConfig(bool bPropagate);
	...
	void STATIC_DumpStateStack();
	void STATIC_PopState(bool bPopAll);
	void STATIC_PushState(const struct FName& NewState, const struct FName& NewLabel);
	struct FName STATIC_GetStateName();
	bool STATIC_IsChildState(const struct FName& TestState, const struct FName& TestParentState);
	bool STATIC_IsInState(const struct FName& TestState, bool bTestStateStack);
	void STATIC_GotoState(const struct FName& NewState, const struct FName& Label, bool bForceEvents, bool bKeepStack);
	...
	void STATIC_logr(const struct FString& S);
	...
	bool STATIC_InCylinder(const struct FVector& Origin, const struct FRotator& Dir, float Width, const struct FVector& A, bool bIgnoreZ);
...
public:
	...
	struct FVector STATIC_GetVectorValue(float F, int LastExtreme);
};
...
public:
	...
	float STATIC_GetFloatValue(float F);
};
