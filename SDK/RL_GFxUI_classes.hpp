...
public:
	...
	bool STATIC_FSCommand(class UGFxMoviePlayer* Movie, class UGFxEvent_FSCommand* Event, const struct FString& Cmd, const struct FString& Arg);
};
...
public:
	...
	void STATIC_NotifyGameSessionEnded();
...
public:
	...
	unsigned long                                      bAllowArrowKeyNav : 1;                                    // 0x0114(0x0004)
	...
	void STATIC_ApplyPriorityEffect(bool bRequestedBlurState, bool bRequestedHiddenState);
	void STATIC_PlaySoundFromTheme(const struct FName& EventName, const struct FName& SoundThemeName);
	void STATIC_OnFocusLost(int LocalPlayerIndex);
	void STATIC_OnFocusGained(int LocalPlayerIndex);
	void STATIC_ConsoleCommand(const struct FString& Command);
	class APlayerController* STATIC_GetPC();
	class ULocalPlayer* STATIC_GetLP();
	...
	bool STATIC_ShouldBlockKey(int ControllerId, const struct FName& ButtonName, TEnumAsByte<EInputEvent> InputEvent);
	bool STATIC_FilterButtonInput(int ControllerId, const struct FName& ButtonName, TEnumAsByte<EInputEvent> InputEvent);
	...
	void STATIC_ConditionalClearPause();
	void STATIC_OnCleanup();
	void STATIC_OnClose();
	void STATIC_Close(bool Unload);
	void STATIC_SetPause(bool bPausePlayback);
	void STATIC_OnPostAdvance(float DeltaTime);
	void STATIC_PostAdvance(float DeltaTime);
	void STATIC_Advance(float Time);
	bool STATIC_Start(bool StartPaused);
};
...
public:
	...
	unsigned long                                      m_bOrigVisible : 1;                                       // 0x00A0(0x0004)
	...
	float                                              m_fOrigAlpha;                                             // 0x00C0(0x0004)
	struct FName                                       m_DebugName;                                              // 0x00C4(0x0008)
	struct FASDisplayInfo                              m_CachedDisplayInfo;                                      // 0x00CC(0x002C)
	TArray<struct FGFxWidgetBinding>                   SubWidgetBindings;                                        // 0x00F8(0x0010) (NeedCtorLink)
	TArray<struct FGFxObjectPoolEntry>                 m_GFxObjectPool;                                          // 0x0108(0x0010) (NeedCtorLink)
	...
	void STATIC_SetDisabled(bool bValue);
	...
	bool STATIC_ReturnPooledMovie(class UGFxObject* pObj);
	class UGFxObject* STATIC_AttachPooledMovie(const struct FString& symbolname, const struct FString& instancename, int Depth, unsigned char byTypeId, class UClass* Type);
...
public:
	...
	bool STATIC_IsValidLevelSequenceObject();
};
...
{
public:
	struct FScriptDelegate                             __EventListener__Delegate;                                // 0x0118(0x000C) (NeedCtorLink)
	unsigned char                                      UnknownData00[0x4];                                       // 0x0118(0x0004) FIX WRONG TYPE SIZE OF PREVIOUS PROPERTY
